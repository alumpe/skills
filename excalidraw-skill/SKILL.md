...
- Use `"text": "My Label"` on shapes to label them. The MCP server auto-converts this to the internal label format.
- Bind arrows with `startElementId` and `endElementId` — they auto-route to element edges.
- **fontFamily**: Must be a string (e.g. `"1"`) or omit entirely. Never pass a number.
